...
def plot_setup(df, ticker, action):
    ...
    fig.add_annotation(x=signal_date, y=y_pos, text=f"{'🟢 AL' if action=='AL' else '🔴 SAT'} KESİŞİMİ", showarrow=True, arrowhead=1, arrowcolor=color, row=1, col=1)
...
def analyze_auto_batch(mkt_key):
    ...
    if is_bull_market:
        ...
        st.warning(regime_msg + " (Makro riskler yüksek, alım sinyalleri temkinli değerlendirilmeli)")
    ...
    st.info(f"Adım 3: {total_symbols} hissenin 1 YILLIK verisi 100'erli paketler halinde indiriliyor...")
        ...
        for ticker in chunk_tickers:
            try:
                ...
                if action:
                    ...
                    # DİKKAT: Artık is_valid koşulu aranmaksızın ekleniyor. Sadece onay durumu kaydediliyor.
                    valid_setups.append({
                        "Sembol": display_symbol,
                        "Sinyal": action,
                        "Fiyat": round(price, 2),
                        "Hacim Katsayısı": f"{current['Volume'] / current['vol_avg_10']:.1f}x",
                        "Onay_Durumu": is_valid,
                        "Nedeni": reason,
                        "SL": round(sl, 2),
                        "TP1": round(tp1, 2),
                        "TP2": round(tp2, 2),
    ...
    # Kullanıcı deneyimi için önce "Tam Onaylıları" üste alalım, "Riskli" olanları alta atalım.
    valid_setups.sort(key=lambda x: x['Onay_Durumu'], reverse=True)
    return valid_setups

# =============================================================================
# 6. KULLANICI ARAYÜZÜ (UI)
# =============================================================================
st.title("🛡️ Profesyonel Kantitatif Tarayıcı (Danışman Modeli)")
st.markdown("Piyasadaki tüm EMA 12/26 kesişimlerini listeler. Ancak SMA 200, MACD ve Hacim teyidi olmayan riskli setup'ları size raporlar.")
st.markdown("---")
...
if start_scan:
    ...
    if not results:
        st.warning("Bu piyasada şu an EMA kesişimi gerçekleştiren hiçbir hisse bulunamadı.")
    else:
        approved_count = sum(1 for r in results if r['Onay_Durumu'])
        risky_count = len(results) - approved_count
        
        st.success(f"Tarama {round(time.time() - start_time, 1)} saniyede tamamlandı! Toplam **{len(results)}** kesişim bulundu. ({approved_count} Tam Onaylı, {risky_count} Riskli/Teyitsiz)")
        ...
        for setup in results:
            ...
            with col_info:
                ...
                # UI UYARI BLOKLARI
                if setup['Onay_Durumu']:
                    st.success("✅ **Sistem Onayı:** Kurumsal Teyitler Sağlandı")
                else:
                    st.warning(f"⚠️ **RİSKLİ:** {setup['Nedeni']}")
                    
                st.metric("İşlem Yönü", setup['Sinyal'])
                st.metric("Giriş Fiyatı", setup['Fiyat'])
                
                # Hacim metriğini rengi de duruma göre değişsin
                delta_color = "normal" if setup['Sinyal'] == "AL" and float(setup['Hacim Katsayısı'].replace('x','')) >= 1.5 else "inverse"
                st.metric("Hacim Şoku (Kurumsal)", setup['Hacim Katsayısı'], delta="Teyitli" if setup['Onay_Durumu'] else "Yetersiz", delta_color=delta_color)
